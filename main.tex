...
\title{Robust-Adaptive Interval Predictive Control for Linear Uncertain Systems}
...
we get:
\begin{gather*}
\int_{t}^{t+T}\Phi^{\top}(s)\Phi(s)ds=\int_{t+g\ell}^{t+T}\Phi^{\top}(s)\Phi(s)ds+\sum_{i=0}^{g-1}\int_{t+i\ell}^{t+(i+1)\ell}\Phi^{\top}(s)\Phi(s)ds\\
\geq\sum_{i=0}^{g-1}\int_{t+i\ell}^{t+(i+1)\ell}\Phi^{\top}(s)\Phi(s)ds\geq\frac{g}{g+1}\frac{\vartheta}{\ell}TI_{d}\geq\frac{\vartheta}{2\ell}TI_{d},
\end{gather*}
